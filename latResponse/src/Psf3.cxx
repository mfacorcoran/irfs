...
double Psf3::angularIntegral(double energy, double theta, 
   ...
   for (size_t i(0); i < 4; i++) {
      yvals.push_back(psf_base_integral(radius, parVectors.at(i)));
   }
   return Bilinear::evaluate(tt, uu, yvals);
}

double Psf3::psf_base_integral(double radius,
                               const std::vector<double> & pars) {
   double ncore(pars.at(0));
   double ntail(pars.at(1));
   double score(pars.at(2));
   double stail(pars.at(3));
